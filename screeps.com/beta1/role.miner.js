    ...
    run: function(creep) {
        ...
        if (creep.memory.transfering) {
            ...
            if (creep.memory.data != null && creep.memory.data.longDistance) {
                // console.log(1);
                if (creep.memory.target == null || creep.memory.target.roomName ==
                    null) {
                    // console.log(2);
                    creep.memory.target = Memory.globalSources[creep.memory
                        .data.globalIndex];
                }
                if (creep.memory.target != null && creep.memory.target.roomName !=
                    null) {
                    ...
                    if (room != null) {
                        // console.log(4);
                        var source = room.lookForAt(LOOK_SOURCES, creep
                            .memory.target.x, creep.memory.target.y
                        );
                        if (source != null) {
                            // console.log(5);
                ...
                var target = Game.getObjectById(creep.memory.target);
                if (target == null || target.energy == 0 || (target.store !=
                        null && target.store[RESOURCE_ENERGY] > 0)) {
