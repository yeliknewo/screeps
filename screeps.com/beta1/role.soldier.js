var roleSoldier = {
    ...
    run: function(creep) {
        ...
        if (creep.memory.attackTarget == null) {
            if (creep.memory.raidTarget == null) {
                ...
                while (choice >= 0) {
                    for (var targetIndex in targetRooms) {
                        ...
                        if (targetRoom != null) {
                            if (creep.memory.usedRooms[targetRoom] !=
                                null && creep.memory.usedRooms[
                                    targetRoom] > Game.time) {
                                continue;
                            }
                            var exitDir = creep.room.findExitTo(
                                targetRoom);
                            var exit = creep.pos.findClosestByPath(
                                exitDir);
                            creep.memory.raidTarget = {
                                roomName: creep.room.name,
                                x: exit.x,
                                y: exit.y
                            };
                            creep.memory.usedRooms[creep.room.name] =
                                Game.time + 100;
                            break;
                        }
                    }
            ...
            if (creep.memory.raidTarget != null) {
                var result = creep.moveTo(creep.room.getPositionAt(
                    creep.memory.raidTarget.x, creep.memory.raidTarget
                    .y), {
                    visualizePathStyle: {
                        stroke: pathColor
                    }
