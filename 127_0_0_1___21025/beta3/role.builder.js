    ...
    run: function(creep) {
        ...
        if(creep.memory.building) {
            ...
            if(target == null || target.hits == target.hitsMax) {
                var newTarget = creep.pos.findClosestByPath(FIND_MY_CONSTRUCTION_SITES);
                if(newTarget != null) {
            ...
            if(target != null && target.progress != null) {
                ...
                if(target == null) {
                    var newTarget = creep.pos.findClosestByPath(FIND_STRUCTURES, {
                        filter: (structure) => {
                            return structure.hits < Math.max(structure.hitsMax - 5000, structure.hitsMax / 2) && (structure.structureType == STRUCTURE_CONTAINER || structure.structureType == STRUCTURE_ROAD);
                        }
                    });
                    if(newTarget != null) {
                        target = newTarget;
                        ...
                        var targets = creep.room.find(FIND_STRUCTURES, {
                            filter: (structure) => {
                                    return structure.hits < 10000 && (structure.structureType == STRUCTURE_RAMPART);
                            }
                        });
                        targets.sort((a, b) => {
                        ...
                        if(targets.length > 0) {
                            ...
                            while(target == null && mult > 0) {
                                var newTarget = creep.pos.findClosestByPath(FIND_STRUCTURES, {
                                    filter: (structure) => {
                                        return structure.hits < structure.hitsMax / mult && structure.structureType != STRUCTURE_ROAD;
                                    }
                                });
                                if(newTarget != null) {
                                    target = newTarget;
